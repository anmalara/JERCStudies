    ...
    if drawLogo:
      ...
      pad.cd()
        ...
        if writeExtraText:
            ...
            if (len(extraText3)!=0):
              latex.SetTextSize(extraTextSize*t*3./5 if extraTextSize3 < 0 else extraTextSize3)
              latex.SetTextFont(extraTextFont3)
              for ind,tt in enumerate(extraText3):
                latex.DrawLatex(posX_, posY_-relExtraDY*cmsTextSize*t - 0.01 - (relExtraDY*extraTextSize*t if len(extraText2)!=0 else 0) -(relExtraDY*extraTextSize*t/2 + 0.02)*(ind+1), tt)
  elif writeExtraText:
    if (outOfFrame):
      posX_ = l + l*(1-l-r)
...
def tdrCanvas(canvName, x_min, x_max, y_min, y_max, nameXaxis, nameYaxis, square=kRectangular, iPos=11, is2D=False, isExtraSpace=False):
  ...
  h.GetXaxis().SetTitleOffset((0.9 if is2D else 0.9)  if square else (1.0 if isExtraSpace else 0.9))
